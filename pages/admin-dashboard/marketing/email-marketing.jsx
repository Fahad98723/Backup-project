...
export default function EmailMarketing() {
  ...
  const sendEmailHandle = async () => {
    const sending = toast.loading("Sending Please wait ...");
    const d = {
      emails: userEmail,
      subject: subject,
      html: body,
    ...
    try {
      ...
      const { data } = await axios.post(`${host}/messaging/email`, d, {
        headers: {
          "Access-Control-Allow-Origin": "*",
  ...
  return (
    <AdminDashboard currentSection="Email Marketing">
      <Head>
        <title>Email Marketing - Admin Dashboard - Ostello</title>
        ...
        <Box sx={style} className={modalBox}>
          <div className=" p-3 ">
            <div className=" w-full mx-auto my-5">
              <p className="text-2xl text-center my-5">Email Marketing</p>
              <div
              ...
              <div
                className={` shrink w-full px-6 py-2 h-[200px] shadow-md rounded-xl text-base font-normal text-slate bg-white bg-clip-padding bg-no-repeat border-2 border-solid border-light-gray first-letter:transition ease-in-out m-0 overflow-x-scroll overflow-y-scroll`}
              >
                <h2 className=" mb-1" style={{ fontSize: "15px" }}>
                  Html Body
                </h2>
